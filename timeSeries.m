...
(* :Discussion:
        ...
        NBER recessions). It provides the ability to do two-axes DateListPlots. Some other
        functions are included, such an HP filter function, and some other basic series
...
twoAxisDateListPlot::usage =
        "twoAxisDateListPlot[{timeSeries1, timeSeries2}, Options] merges two DateListPlots into a \
single one with two y axis. The option \"2ndAxisTicks\" provides a list with the ticks for \
the second axis (defaults to Automatic). The option \"HasLegend\" takes either a value of True or \
False, and tells whether the plots contain a legend (defaults to False). \"OptionsForPlot1\" \
provides the options for the first plot, and \"OptionsForPlot2\" provides the options for \
the second. \"OptionsForAll\" provides the options for the final Show command that combines \
the two plots.";
...
getTrendHPFilter[ts_?(ArrayQ[#, 1] &), smooth : _?NumericQ : 1600., OptionsPattern[]] := With[
        {
                T = Length[ts],
                alpha = N[smooth],
...
Options[twoAxisDateListPlot] = {
        "HasLegend" -> False,
        "OptionsForPlot1" -> {},
        "OptionsForPlot2" -> {},
        "OptionsForAll" -> {},
        "2ndAxisTicks" -> Automatic
};

twoAxisDateListPlot[{timeseries1_, timeseries2_},
(* This function is a modification of the function here:
   http://reference.wolfram.com/mathematica/howto/GeneratePlotsWithTwoVerticalScales.html
*)
        OptionsPattern[]] :=
        Module[
                {
                ...
                With[
                        {
                                hasLegend = OptionValue["HasLegend"],
                                optionsForPlot1 = OptionValue["OptionsForPlot1"],
                                optionsForPlot2 = OptionValue["OptionsForPlot2"],
                                generalOptions = OptionValue["OptionsForAll"],
                                axis2ticksOption = OptionValue["2ndAxisTicks"]
                        },
                        {fgraph, ggraph} = Map[DateListPlot[#[[1]], #1[[2]]] &,
                                {{timeseries1, optionsForPlot1}, {timeseries2, optionsForPlot2}}];
                        {frange, grange} = (PlotRange /.
                        ...
                        With[
                                ...
                                gticks = MapThread[
                                        {
                                                #2,
                                                ToString[NumberForm[If[IntegerQ[#1], #1, N[#1]], 2], StandardForm]
                                        } &,
                                ...
                                FrameTicks -> {
                                        {
                                                Options[
                                                        If[hasLegend,
                                                                First@fgraph, (* Plots with Legends have a different structure *)
                                                                fgraph],
                                                        FrameTicks][[1, 2, 1, 1]],
                                                gticks
                                        },
                                        Options[If[hasLegend, First@fgraph, fgraph], FrameTicks][[1, 2, 2]]
                                },
                                generalOptions
                        ]
                ]
...
Options[mapThreadTimeSeries] = {"IgnoreMissingHead" -> False}
mapThreadTimeSeries[f_, ts_, OptionsPattern[]] :=
        Module[{date},
                First@Rest[
                        Reap[
                                Apply[Sow[#2, date[#1]]&, ts, {2}],
                                Alternatives @@ date /@ Intersection @@ ts[[All, All, 1]],
                                If[OptionValue["IgnoreMissingHead"],
                                        {#1[[1]], f @@ #2},
                                        If[FreeQ[#2, Missing],
                                                {#1[[1]], f @@ #2},
                                                {#1[[1]], Missing[]}]
